<!-- <tr>
  ...
  <td>
    <form method="POST" action="">
      <button type="submit" name="update-id" value="" class="update-btn">
...
$result = mysqli_query($conn, "SELECT * FROM view_issue_items");
        ...
        <tr>
          ...
          <td>
            <select name='isu-type' id='isuSel'>
              <option hidden>{$row['issue']}</option>
          ...
          <!-- Issue Texts -->
          <td>
            <input type='text' id='isuTxt' value='{$row['text']}'>
          </td>
          <!-- Quantity Control Buttons -->
          <td class='qty-ctrl'>
            <button class='qty-ctrl-btn minus-btn' data-action='-'>-</button>
            <input type='number' id='old-qty' class='qty-ctrl-inp' value='{$row['qty']}' />
            <button class='qty-ctrl-btn plus-btn' data-action='+'>+</button>
            <input type='number' value='{$row['quantity']}' id='maxQty' hidden>
          </td>

          <!-- Action Buttons -->
          <td>
            <form method='POST' action=''>
              <button type='submit' name='update-id' value='{$row['issue_id']}' class='update-btn'>
              ...
              <button type='submit' name='delete-id' value='{$row['issue_id']}' class='delete-btn'>
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor' class='size-6 icon'>
              ...
              <!-- Hidden Inputs for backend purpose -->
              <input type='text' name='isu-type' id='new-isuSel' value='{$row['issue']}' hidden/>
              <input type='text' id='new-isuTxt' name='isu-text' value='{$row['text']}' hidden/>
              <input type='number' name='qty' id='new-qty' value='{$row['qty']}' hidden/>
            </form>
          </td>
        </tr>
      ";
